...
impl::mdarray2_t<T> make_serendipity_space_2d(int degree)
{
  ...
  impl::mdspan3_t<const T> Pq(_Pq.data(), shape);
...
//----------------------------------------------------------------------------
template <std::floating_point T>
impl::mdarray2_t<T> make_serendipity_space_3d(int degree)
{
  ...
  impl::mdspan3_t<const T> Ph(_Ph.data(), shape);
...
//----------------------------------------------------------------------------
template <std::floating_point T>
impl::mdarray2_t<T> make_serendipity_div_space_2d(int degree)
{
  ...
  impl::mdarray2_t<T> wcoeffs(ndofs, psize * 2);
  int row_n = 0;
  for (int i = 0; i <= degree; ++i)
    for (int j = 0; j <= degree - i; ++j)
      for (int d = 0; d < 2; ++d)
...
//----------------------------------------------------------------------------
template <std::floating_point T>
impl::mdarray2_t<T> make_serendipity_div_space_3d(int degree)
{
  ...
  // Create coefficients for order (degree) vector polynomials
  impl::mdarray2_t<T> wcoeffs(ndofs, psize * 3);
  int row_n = 0;
  for (int i = 0; i <= degree; ++i)
  {
    for (int j = 0; j <= degree - i; ++j)
...
//----------------------------------------------------------------------------
template <std::floating_point T>
impl::mdarray2_t<T> make_serendipity_curl_space_2d(int degree)
{
  const std::size_t ndofs = degree * (degree + 3) + 4;
...
//----------------------------------------------------------------------------
template <std::floating_point T>
impl::mdarray2_t<T> make_serendipity_curl_space_3d(int degree)
{
                                  ...
                                  bool discontinuous)
{
  ...
  const auto [_phi, shape] = polynomials::tabulate(polynomials::type::legendre,
                                                   celltype, degree, pts);
  impl::mdspan2_t<const T> phi(_phi.data(), shape);
...
//-----------------------------------------------------------------------------
template <std::floating_point T>
impl::mdarray2_t<T> make_dpc_points(cell::type celltype, int degree,
                                    element::dpc_variant variant)
{
  auto to_mdspan = [](auto& x, auto shape)
  { return impl::mdspan2_t<const T>(x.data(), shape); };
  auto to_mdarray = [](auto& x, auto shape)
  { return impl::mdarray2_t<T>(x, shape[0], shape[1]); };
           ...
           or variant == element::dpc_variant::horizontal_gll)
  {
    ...
    case cell::type::quadrilateral:
    {
      impl::mdarray2_t<T> pts((degree + 2) * (degree + 1) / 2, 2);
      std::size_t n = 0;
    ...
    case cell::type::hexahedron:
    {
      impl::mdarray2_t<T> pts((degree + 3) * (degree + 2) * (degree + 1) / 6,
                              3);
      std::size_t n = 0;
                                          ...
                                          bool discontinuous)
{
  ...
  else
  {
    ...
    x[1] = std::vector(num_ent, impl::mdarray2_t<T>(0, tdim));
    M[1] = std::vector(num_ent, impl::mdarray4_t<T>(0, 1, 0, 1));
  }
  ...
  if (tdim >= 2)
  {
    ...
    else
    {
      ...
      M[2] = std::vector(num_ent, impl::mdarray4_t<T>(0, 1, 0, 1));
    }
  }
  ...
  if (tdim == 3)
  {
    ...
    else
    {
      ...
      M[3] = std::vector(num_ent, impl::mdarray4_t<T>(0, 1, 0, 1));
    }
  }
  ...
  return FiniteElement(element::family::serendipity, celltype, degree, {},
                       impl::mdspan2_t<const T>(wbuffer.data(), wshape), xview,
                                  ...
                                  bool discontinuous)
{
  ...
  const impl::mdarray2_t<T> pt = make_dpc_points<T>(celltype, degree, variant);
  x[tdim].push_back(pt);

  return FiniteElement(
      element::family::DPC, celltype, degree, {},
      impl::mdspan2_t<T>(wcoeffs.data(), wcoeffs.extents()), impl::to_mdspan(x),
    ...
    element::dpc_variant dvariant, bool discontinuous)
{
  ...
  impl::mdspan2_t<const T> Qpts(_Qpts.data(), wts.size(),
                                _Qpts.size() / wts.size());
  ...
  std::array<std::vector<impl::mdarray4_t<T>>, 4> M;
  {
    ...
    M[0] = std::vector(num_ent, impl::mdarray4_t<T>(0, tdim, 0, 1));
  }
