                                           ...
                                           cell::type cell, int degree)
{
      ...
      double kappa_tol)
  {
    ...
    for (std::size_t d = 0; d < M.size(); ++d)
    {
      ...
      for (std::size_t e = 0; e < x[d].size(); ++e)
      {
        ...
        {
          P = xt::view(polyset::tabulate(celltype, degree, 0, x_e), 0,
                       xt::all(), xt::all());
        }
  ...
  FiniteElement::FiniteElement(
      ...
      : map_type(map_type), _cell_type(cell_type),
        ...
        _cell_subentity_types(cell::subentity_types(cell_type)),
        _family(family), _degree(degree), _map_type(map_type),
        ...
        _discontinuous(discontinuous)
  {
    ...
    {
      for (std::size_t i = 0;
           _dof_transformations_are_permutations and i < et.second.shape(0);
           ++i)
      {
  ...
  //-----------------------------------------------------------------------------
  const std::vector<std::vector<std::set<int>>>& FiniteElement::entity_dofs()
      const
  {
    return _edofs;
  }
  //-----------------------------------------------------------------------------
  const std::vector<std::vector<int>>& FiniteElement::num_entity_closure_dofs()
      const
  {
  ...
  //-----------------------------------------------------------------------------
  xt::xtensor<double, 4> FiniteElement::tabulate(
      int nd, const xt::xarray<double>& x) const
  {
    std::size_t ndsize = 1;
    for (int i = 1; i <= nd; ++i)
      ndsize *= (_cell_tdim + i);
                               ...
                               xt::xtensor<double, 4>& basis_data) const
  {
    ...
    xt::xtensor<double, 3> basis
        = polyset::tabulate(_cell_type, _degree, nd, _x);
  ...
  xt::xtensor<double, 3> FiniteElement::map_push_forward(
      ...
      const xtl::span<const double>& detJ, const xt::xtensor<double, 3>& K)
      const
  {
    ...
    if (_cell_tdim >= 2)
    {
      ...
      for (std::size_t e = 0; e < _num_edofs[1].size(); ++e)
      {
        ...
        if (cell_info >> (face_start + e) & 1)
          precompute::apply_permutation(_eperm.at(cell::type::interval)[0],
                                        dofs, dofstart);
