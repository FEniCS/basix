...
bool add_syslog(const char* app_name, Verbosity verbosity, int facility)
{
  ...
  (void)verbosity;
  (void)facility;
  VLOG_F(g_internal_verbosity, "syslog not implemented on this system. Request "
...
void add_stack_cleanup(const char* find_this, const char* replace_with_this)
{
  ...
  {
    LOG_F(WARNING, "add_stack_cleanup: the replacement should be shorter than "
                   "the pattern!");
    return;
  }
...
#else
      auto prefix = textprintf("[ErrorContext] %*s:%-5u %-20s ",
                               LOGURU_FILENAME_WIDTH, filename(entry->_file),
                               entry->_line, description.c_str());
#endif
