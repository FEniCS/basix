                ...
                const xt::xtensor<double, 2>& K)
{
  ...
  const xt::xtensor<double, 1> r
      = basix::dot21(xt::transpose(K), _reference_data);
                    ...
                    const xt::xtensor<double, 2>& /*K*/)
{
  ...
  auto _reference_data = xt::adapt(reference_data.data(), reference_data.size(),
                                   xt::no_ownership(), s);
  const xt::xtensor<double, 1> r = 1 / detJ * basix::dot21(J, _reference_data);
  return std::vector<double>(r.begin(), r.end());
}
//-----------------------------------------------------------------------------
std::vector<double>
                       ...
                       const xt::xtensor<double, 2>& K)
{
  ...
  xt::xtensor<double, 2> r
      = basix::dot22(xt::transpose(K), basix::dot22(data_matrix, K));
  return std::vector<double>(r.begin(), r.end());
}
//-----------------------------------------------------------------------------
std::vector<double>
                           ...
                           const xt::xtensor<double, 2>& /*K*/)
{
  ...
  xt::xtensor<double, 2> r
      = 1 / (detJ * detJ)
        * basix::dot22(J, basix::dot22(data_matrix, xt::transpose(J)));
  return std::vector<double>(r.begin(), r.end());
}
//-----------------------------------------------------------------------------
} // namespace
