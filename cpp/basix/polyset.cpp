  ...
  for (std::size_t ky = 1; ky <= nderiv; ++ky)
  {
    for (std::size_t kz = 0; kz <= nderiv - ky; ++kz)
    {
      for (std::size_t p = 1; p <= n; ++p)
      {
        ...
        {
          p00 += ky * a
                 * xt::view(P, idx(0, ky - 1, kz), xt::all(), idx(p - 1, 0, 0));
        }

        if (kz > 0)
        {
          p00 += kz * a
                 * xt::view(P, idx(0, ky, kz - 1), xt::all(), idx(p - 1, 0, 0));
        }
      ...
      for (std::size_t p = 0; p < n; ++p)
      {
        ...
        if (kz > 0)
        {
          p10 += kz * xt::view(P, idx(0, ky, kz - 1), xt::all(), idx(p, 0, 0));
        }
        ...
        for (std::size_t q = 1; q < n - p; ++q)
        {
          ...
          if (kz > 0)
          {
            pq1 += kz * xt::view(P, idx(0, ky, kz - 1), xt::all(), idx(p, q, 0))
                       * (aq - bq)
                   + kz * (1.0 - (x2 * 2.0 - 1.0))
                         * xt::view(P, idx(0, ky, kz - 1), xt::all(),
                                    idx(p, q - 1, 0))
        ...
        for (std::size_t q = 0; q + 1 < n - p; ++q)
        {
          for (std::size_t r = 1; r < n - p - q; ++r)
          {
            ...
            if (kz > 0)
            {
              xt::view(P, idx(0, ky, kz), xt::all(), idx(p, q, r + 1))
                  += 2 * kz * ar
                     * xt::view(P, idx(0, ky, kz - 1), xt::all(), idx(p, q, r));
            }
          }
        }
      }
