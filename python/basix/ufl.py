...
class _ElementBase(_AbstractFiniteElement):
    ...
    @_abstractmethod
    def tabulate(
        self, nderivs: int, points: _npt.NDArray[np.float64]
    ) -> _npt.NDArray[np.float64]:
    ...
    ) -> _typing.Tuple[_typing.Any, int, int]:
    ...
    def custom_quadrature(
        self,
    ) -> _typing.Tuple[_npt.NDArray[np.float64], _npt.NDArray[np.float64]]:
    ...
    @property
    def _x(self) -> _typing.List[_typing.List[_npt.NDArray[np.float64]]]:
        """The points used to define interpolation."""
        raise NotImplementedError()

    @property
    def _M(self) -> _typing.List[_typing.List[_npt.NDArray[np.float64]]]:
...
class _BasixElement(_ElementBase):
    ...
    def __init__(
        ...
        if element.family == _basix.ElementFamily.custom:
            ...
            repr = (
                f"Basix element ({element.family.__name__}, {element.cell_type.__name__}, {element.degree}, "
    ...
    def __eq__(self, other) -> bool:
        ...
        return isinstance(other, _BasixElement) and (
            self.element == other.element and self._gdim == other._gdim
        )
    ...
    def get_component_element(
        self, flat_component: int
    ) -> _typing.Tuple[_ElementBase, int, int]:
        """Get element that represents a component of the element, and the offset and stride of the component.
...
class _ComponentElement(_ElementBase):
    ...
    def __init__(
        self, element: _ElementBase, component: int, gdim: _typing.Optional[int] = None
    ):
        """Initialise the element."""
        self.element = element
        ...
        repr += ")"
        super().__init__(
            repr, element.cell_type.__name__, (1,), element._degree, gdim=gdim
        )
    ...
    def tabulate(
        ...
        for tbl in tables:
            ...
            if len(self.element._value_shape) == 0:
                ...
                if (
                    isinstance(self.element, _BlockedElement)
                    and self.element._has_symmetry
                ):
...
class _MixedElement(_ElementBase):
    ...
    def __init__(
        self,
        sub_elements: _typing.List[_ElementBase],
        gdim: _typing.Optional[int] = None,
    ):
        """Initialise the element."""
    ...
    def tabulate(
        ...
        for deriv_tables in zip(*results):
            new_table = np.zeros((len(points), self.value_size * self.dim))
            start = 0
            for e, t in zip(self._sub_elements, deriv_tables):
                for i in range(0, e.dim, e.value_size):
                    new_table[:, start : start + e.value_size] = t[
                        :, i : i + e.value_size
                    ]
...
class _BlockedElement(_ElementBase):
    ...
    def __init__(
        ...
        super().__init__(
            repr,
            sub_element.cell_type.__name__,
            shape,
            sub_element._degree,
    ...
    def tabulate(
        ...
        for table in self.sub_element.tabulate(nderivs, points):
            ...
            new_table = np.zeros(
                (table.shape[0], *self.block_shape, self._block_size * table.shape[1])
            )
            for i, j in enumerate(
                _itertools.product(*[range(s) for s in self.block_shape])
            ):
                if len(j) == 1:
    ...
    def reference_value_shape(self) -> _typing.Tuple[int, ...]:
        ...
        if self._has_symmetry:
            assert (
                len(self.block_shape) == 2
                and self.block_shape[0] == self.block_shape[1]
            )
    ...
    def num_entity_dofs(self) -> _typing.List[_typing.List[int]]:
        """Number of DOFs associated with each entity."""
        return [
            [j * self._block_size for j in i] for i in self.sub_element.num_entity_dofs
        ]
    ...
    def entity_dofs(self) -> _typing.List[_typing.List[_typing.List[int]]]:
        ...
        return [
            ...
            for i in self.sub_element.entity_dofs
        ]
    ...
    def num_entity_closure_dofs(self) -> _typing.List[_typing.List[int]]:
        ...
        return [
            ...
            for i in self.sub_element.num_entity_closure_dofs
        ]
    ...
    def entity_closure_dofs(self) -> _typing.List[_typing.List[_typing.List[int]]]:
        ...
        return [
            ...
            for i in self.sub_element.entity_closure_dofs
        ]
...
class _RealElement(_ElementBase):
    ...
    def __init__(self, cell: _basix.CellType, value_shape: _typing.Tuple[int, ...]):
        ...
        super().__init__(
            f"RealElement({cell.__name__}, {value_shape})",
            cell.__name__,
            value_shape,
            0,
        )
...
def _compute_signature(element: _basix.finite_element.FiniteElement) -> str:
    ...
    signature = (
        f"{element.cell_type.__name__}, {element.value_shape}, {element.map_type.__name__}, "
...
def element(
    ...
    e = _basix.create_element(
        family, cell, degree, lagrange_variant, dpc_variant, discontinuous
    )
...
def enriched_element(
    elements: _typing.List[_ElementBase],
...
def custom_element(
    ...
    x: _typing.List[_typing.List[_npt.NDArray[np.float64]]],
    M: _typing.List[_typing.List[_npt.NDArray[np.float64]]],
...
def mixed_element(
    elements: _typing.List[_ElementBase], gdim: _typing.Optional[int] = None
) -> _ElementBase:
...
def real_element(
    cell: _typing.Union[_basix.CellType, str], value_shape: _typing.Tuple[int, ...]
) -> _ElementBase:
