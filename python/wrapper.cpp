...
Interface to the Basix C++ library.
  ...
  //       cell::facet_outward_normals(cell_type); return
  //       py::array_t<double>(normals.shape(), normals.data());
  //     },
  ...
  //             int block_size, std::uint32_t cell_info) {
  //            xtl::span<double> data_span(data.mutable_data(), data.size());
  //            self.apply_dof_transformation_to_transpose(data_span,
  //            block_size,
  //                                                       cell_info);
  //            return py::array_t<double>(data_span.size(), data_span.data());
  //          })
  ...
  //            for (auto tpart : t)
  //            {
  //              t2[cell::type_to_str(tpart.first).c_str()] =
  //              py::array_t<double>(
  ...
  //     &FiniteElement::num_entity_dofs) .def_property_readonly("entity_dofs",
  //     &FiniteElement::entity_dofs)
  ...
  //        int degree) -> FiniteElement { return
  //        basix::create_element(family_name, cell_name, degree);
  //     },
  ...
  // m.def("index", py::overload_cast<int>(&basix::idx), "Indexing for 1D
  // arrays")
