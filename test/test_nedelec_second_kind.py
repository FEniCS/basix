...
def test_tri(order):
    ...
    x = sympy.Symbol("x")
    y = sympy.Symbol("y")
    nedelec = basix.create_element(basix.ElementFamily.n2e, basix.CellType.triangle, order)
...
def test_tet(order):
    ...
    y = sympy.Symbol("y")
    z = sympy.Symbol("z")
    nedelec = basix.create_element(basix.ElementFamily.n2e, basix.CellType.tetrahedron, order)
    pts = basix.create_lattice(celltype, 6, basix.LatticeType.equispaced, True)
    nderiv = 1
    wtab = nedelec.tabulate(nderiv, pts)
