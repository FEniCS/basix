...
def test_cell_quadrature(celltype, order):
    Qpts, Qwts = basix.make_quadrature(celltype[0], basix.PolysetType.standard, order)
...
def test_qorder_line(m, scheme):
    Qpts, Qwts = basix.make_quadrature(scheme, basix.CellType.interval, basix.PolysetType.standard, m)
    x = sympy.Symbol('x')
    f = x**m
...
def test_qorder_tri(m, scheme):
    Qpts, Qwts = basix.make_quadrature(scheme, basix.CellType.triangle, basix.PolysetType.standard, m)
    x = sympy.Symbol('x')
    y = sympy.Symbol('y')
    f = x**m + y**m
...
def test_xiao_gimbutas_tet(m, scheme):
    Qpts, Qwts = basix.make_quadrature(scheme, basix.CellType.tetrahedron, basix.PolysetType.standard, m)
    x = sympy.Symbol('x')
    y = sympy.Symbol('y')
    z = sympy.Symbol('z')
    f = x**m + 2 * y**m + 3 * z**m
...
def test_quadrature_function():
    Qpts, Qwts = basix.make_quadrature(basix.CellType.interval, basix.PolysetType.standard, 3)
...
def test_gll():
    ...
    # 1D interval
    pts, wts = basix.make_quadrature(basix.QuadratureType.gll, basix.CellType.interval, basix.PolysetType.standard, m+1)
